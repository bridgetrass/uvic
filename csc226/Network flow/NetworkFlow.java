...
public class NetworkFlow{
	...
	*/
	static int[][] MaxFlow(int[][] G){
		...
		int u,v;
		int [][] rGraph= new int[numVerts+1][numVerts+1];
		int [][] flowG= new int[numVerts+1][numVerts+1];
		// build residual graph (copy G), initialize flow graph to 0
		...
		while(BFS(rGraph, 0, 1, parent)){
			int bottle = Integer.MAX_VALUE;
			for (v=1; v!=0; v=parent[v]){
				u = parent[v];
				bottle = Math.min(bottle, rGraph[u][v]);
			}
	...
	// If G > 0 then flow[u][v] += bottle
	// else flow[v][u] -= bottle
	
	

	
	public static boolean BFS(int[][] G, int start, int goal, int [] parent){
		int numVerts = G.length;
