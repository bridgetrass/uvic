...
public class AVLTree{
	...
	*/
	...
	public static final boolean TestBSTAfterEachInsertion = true;
	...
	*/
	public TreeNode insert(String s){
		...
		if(root==null){
			...
			while(current!=null){
				parse=current;
				int c = s.compareTo(parse.nodeValue);
					if(c<0){
	...
	//node is root
		if(node==root){
			...
			if(node.leftChild!=null){
				TreeNode m = max(node.rightChild);
				root.nodeValue= m.nodeValue;
				m.parent.rightChild=null;
				return;
			}
			else if(node.rightChild!=null){
				TreeNode m = min(node.rightChild);
				root.nodeValue= m.nodeValue;
		...
		int c = node.nodeValue.compareTo(node.parent.nodeValue);
		...
		//node has 2 children
			...
			TreeNode m = min(node.rightChild);
			int l = m.nodeValue.compareTo(m.parent.nodeValue);
			node.nodeValue= m.nodeValue;
			if(l<0){
				if(m.rightChild!=null){
					m.rightChild.parent = m.parent;
					m.parent.leftChild = m.rightChild;
				}
				else{
					m.parent.leftChild=null;
				}
